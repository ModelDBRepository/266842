...
#define nrn_init _nrn_init__GoC
#define _nrn_initial _nrn_initial__GoC
#define nrn_cur _nrn_cur__GoC
#define _nrn_current _nrn_current__GoC
#define nrn_jacob _nrn_jacob__GoC
#define nrn_state _nrn_state__GoC
#define _net_receive _net_receive__GoC 
#define state state__GoC 
...
#define g_gaba _p[8]
#define Dv_GoC _p[9]
#define Dg_ampa _p[10]
#define Dg_gaba _p[11]
#define v _p[12]
#define _g _p[13]
#define _tsav _p[14]
...
#define Cm Cm_GoC
 double Cm = 50;
#define Grest Grest_GoC
 double Grest = 3;
#define epas epas_GoC
 double epas = -65;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 GoC D:/Projects/SchreglmannEtAl2020/CCTC_model/modfiles/grl_goc.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Golgi cell in the granular layer (GrL)";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dv_GoC = ( i_ampa + i_gaba + Grest * ( epas - v_GoC ) ) / Cm ;
 ...
 Dv_GoC = Dv_GoC  / (1. - dt*( ( ( ( Grest )*( ( ( - 1.0 ) ) ) ) ) / Cm )) ;
 ...
 static int state (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    v_GoC = v_GoC + (1. - exp(dt*(( ( ( Grest )*( ( ( - 1.0 ) ) ) ) ) / Cm)))*(- ( ( ( i_ampa + i_gaba + ( Grest )*( ( epas ) ) ) ) / Cm ) / ( ( ( ( Grest )*( ( ( - 1.0 ) ) ) ) ) / Cm ) - v_GoC) ;
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 g_gaba = g_gaba + _args[0] ;
   ...
   if ( _args[0] >= 0.01  && _args[0] <= 0.03  && v_GoC > - 50.0 ) {
       if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = v_GoC;
    double __primary = (20.0) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( ( ( Grest )*( ( ( - 1.0 ) ) ) ) ) / Cm ) ) )*( - ( ( ( i_ampa + i_gaba + ( Grest )*( ( epas ) ) ) ) / Cm ) / ( ( ( ( Grest )*( ( ( - 1.0 ) ) ) ) ) / Cm ) - __primary );
    v_GoC += __primary;
  } else {
 v_GoC = 20.0 ;
       }
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(v_GoC) - _p;  _dlist1[0] = &(Dv_GoC) - _p;
 _slist1[1] = &(g_ampa) - _p;  _dlist1[1] = &(Dg_ampa) - _p;
 _slist1[2] = &(g_gaba) - _p;  _dlist1[2] = &(Dg_gaba) - _p;
_first = 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "grl_goc.mod";
static const char* nmodl_file_text = 
